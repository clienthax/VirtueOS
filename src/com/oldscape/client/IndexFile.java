...
public final class IndexFile {
		...
		synchronized (this.dataFile) {
			try {
				...
				if (this.indexFile.length() < var1 * 6 + 6) {
					...
					int var3 = ((IndexStore_buffer[0] & 255) << 16) + (IndexStore_buffer[2] & 255)
							+ ((IndexStore_buffer[1] & 255) << 8);
					int var4 = (IndexStore_buffer[5] & 255) + ((IndexStore_buffer[3] & 255) << 16)
							+ ((IndexStore_buffer[4] & 255) << 8);
					if (var3 < 0 || var3 > this.maxSize) {
						...
						while (var6 < var3) {
							...
							int var11 = ((IndexStore_buffer[5] & 255) << 8) + ((IndexStore_buffer[4] & 255) << 16)
									+ (IndexStore_buffer[6] & 255);
		...
		synchronized (this.dataFile) {
			try {
				...
				if (var4) {
					...
					this.indexFile.seek(var1 * 6);
					this.indexFile.read(IndexStore_buffer, 0, 6);
					var6 = (IndexStore_buffer[5] & 255) + ((IndexStore_buffer[3] & 255) << 16)
							+ ((IndexStore_buffer[4] & 255) << 8);
					...
					if (var7 < var3) {
						label145: {
							...
							if (var4) {
								...
								var9 = ((IndexStore_buffer[5] & 255) << 8) + ((IndexStore_buffer[4] & 255) << 16)
										+ (IndexStore_buffer[6] & 255);
								int var12 = IndexStore_buffer[7] & 255;
	...
	public static KitDefinition getKitDefinition(int var0) {
		KitDefinition var1 = (KitDefinition) KitDefinition.identKits.get(var0);
		if (var1 != null) {
			return var1;
		} else {
			...
			KitDefinition.identKits.put(var1, var0);
			return var1;
		}
	}
}
