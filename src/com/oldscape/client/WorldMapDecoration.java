...
public class WorldMapDecoration {
	...
	public static final SpritePixels createSprite(int var0, int var1, int var2, int var3, int var4, boolean var5) {
		...
		long var6 = ((long) var1 << 16) + var0 + ((long) var2 << 38) + ((long) var4 << 40) + ((long) var3 << 42);
		SpritePixels var8;
		if (!var5) {
		...
		if (var19 == null) {
			...
			var19.method2734(0, var9.yan2d, var9.zan2d, var9.xan2d, var9.offsetX2d,
					var19.modelHeight / 2 + var17 + var9.offsetY2d, var18 + var9.offsetY2d);
			if (var9.notedId != -1) {
				var20.drawAt(0, 0);
			}
	...
	static final void method314(int var0, int var1, int var2, int var3) {
		...
		for (GraphicsObject var4 = (GraphicsObject) Client.graphicsObjectDeque
				.getFront(); var4 != null; var4 = (GraphicsObject) Client.graphicsObjectDeque.getNext()) {
		...
		if (!Client.field1111) {
			...
			if (Client.field960 == 1 && Client.rights >= 2 && Client.gameCycle % 50 == 0
					&& (class169.field2228 >> 7 != SoundTaskDataProvider.localPlayer.x >> 7
							|| class46.field578 >> 7 != SoundTaskDataProvider.localPlayer.y >> 7)) {
		...
		if (!Client.field1111) {
			if (Client.preferences.hideRoofs) {
				...
				label680: {
					...
					if (SoundTaskDataProvider.localPlayer.x >= 0 && SoundTaskDataProvider.localPlayer.y >= 0
							&& SoundTaskDataProvider.localPlayer.x < 13312
							&& SoundTaskDataProvider.localPlayer.y < 13312) {
						if ((class62.tileSettings[BoundingBox3DDrawMode.plane][SoundTaskDataProvider.localPlayer.x >> 7][SoundTaskDataProvider.localPlayer.y >> 7]
								& 4) != 0) {
							var6 = BoundingBox3DDrawMode.plane;
						}
		...
		for (var10 = 0; var10 < 5; ++var10) {
			if (Client.field955[var10]) {
				var11 = (int) (Math.random() * (Client.field942[var10] * 2 + 1) - Client.field942[var10]
						+ Math.sin(Client.field1116[var10] * (Client.field939[var10] / 100.0D))
								* Client.field1006[var10]);
				if (var10 == 0) {
		...
		class255.region.drawRegion(Player.cameraX, GameObject.cameraZ, class20.cameraY, GrandExchangeOffer.cameraPitch,
				class28.cameraYaw, var28);
			...
			if (var29 == null) {
				...
				for (var18 = 0; var18 < Client.overheadTextCount; ++var18) {
					...
					while (var23) {
						...
						for (int var24 = 0; var24 < var18; ++var24) {
							if (var20 + 2 > Client.overheadTextsY[var24] - Client.overheadTextsOffsetY[var24]
									&& var20 - var22 < Client.overheadTextsY[var24] + 2
									&& var31 - var21 < Client.overheadTextsOffsetX[var24] + Client.overheadTextsX[var24]
									&& var21 + var31 > Client.overheadTextsX[var24] - Client.overheadTextsOffsetX[var24]
									&& Client.overheadTextsY[var24] - Client.overheadTextsOffsetY[var24] < var20) {
					...
					if (Client.field1031 == 0) {
						...
						if (Client.field963[var18] == 0) {
							MessageNode.fontBold12.drawTextCentered(var32, var0 + Client.screenX, Client.screenY + var1,
									var25, 0);
						}

						if (Client.field963[var18] == 1) {
							MessageNode.fontBold12.method5515(var32, var0 + Client.screenX, Client.screenY + var1,
									var25, 0, Client.field1137);
						}

						if (Client.field963[var18] == 2) {
							MessageNode.fontBold12.method5516(var32, var0 + Client.screenX, Client.screenY + var1,
									var25, 0, Client.field1137);
						}

						if (Client.field963[var18] == 3) {
							MessageNode.fontBold12.method5517(var32, var0 + Client.screenX, Client.screenY + var1,
									var25, 0, Client.field1137, 150 - Client.overheadTextsCyclesRemaining[var18]);
						}

						if (Client.field963[var18] == 4) {
							var26 = (150 - Client.overheadTextsCyclesRemaining[var18])
									* (MessageNode.fontBold12.getTextWidth(var32) + 100) / 150;
							Rasterizer2D.setInnerDrawRegion(var0 + Client.screenX - 50, var1,
									var0 + Client.screenX + 50, var3 + var1);
							MessageNode.fontBold12.method5510(var32, var0 + Client.screenX + 50 - var26,
									Client.screenY + var1, var25, 0);
							Rasterizer2D.setDrawRegion(var0, var1, var0 + var2, var3 + var1);
						}
						...
						if (Client.field963[var18] == 5) {
							...
							Rasterizer2D.setInnerDrawRegion(var0,
									Client.screenY + var1 - MessageNode.fontBold12.verticalSpace - 1, var0 + var2,
									Client.screenY + var1 + 5);
							MessageNode.fontBold12.drawTextCentered(var32, var0 + Client.screenX,
									var27 + Client.screenY + var1, var25, 0);
							Rasterizer2D.setDrawRegion(var0, var1, var0 + var2, var3 + var1);
						}
					} else {
						MessageNode.fontBold12.drawTextCentered(var32, var0 + Client.screenX, Client.screenY + var1,
				...
				if (Client.field974) {
					if (Client.cursorState == 1) {
						class248.crossSprites[Client.field972 / 100].drawAt(Client.lastLeftClickX - 8,
								Client.lastLeftClickY - 8);
					}

					if (Client.cursorState == 2) {
						class248.crossSprites[Client.field972 / 100 + 4].drawAt(Client.lastLeftClickX - 8,
								Client.lastLeftClickY - 8);
					}
				}
				...
				if (Client.field880) {
					byte var34 = 0;
					var14 = var34 + class264.NetCache_pendingPriorityWritesCount
